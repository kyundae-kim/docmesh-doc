...
이 문서는 현재 `create_application()`과 고정된 dependency가 읽는 환경 설정을 정의한다. 저장소의 `.env.example`은 PostgreSQL metadata store와 MinIO object store 조합을 선택한 배포 template이다.

- 애플리케이션 코드는 metadata backend를 강제하지 않는다. `dms.create_sdk_from_environment(dict(os.environ))`가 backend를 선택하며 `.env.example`은 `DMS_METADATA_BACKEND=postgresql`을 사용한다.
- object store는 현재 DMS 환경 factory가 구성하는 MinIO다.
- 필수 `dms` readiness check는 `sdk.check_health().ok`를 평가한다. startup health check는 `DOCMESH_HEALTHCHECK_ENABLED` 설정에 따르며 기본 애플리케이션 코드가 `true`를 강제하지 않는다.
...
운영 환경에서 `POSTGRES_PASSWORD`, `MINIO_SECRET_KEY`, `KEYCLOAK_CLIENT_SECRET`의 기본값을 두거나 source code에 기록해서는 안 된다.
...
| 변수 | 필수 | 예시 형식 | 설명 |
| --- | --- | --- | --- |
| `POSTGRES_HOST` | 예 | `postgres` | PostgreSQL host |
| `POSTGRES_PORT` | 아니오 | `5432` | PostgreSQL port. 기본값 5432 |
| `POSTGRES_DB` | 예 | `docmesh` | PostgreSQL database |
| `POSTGRES_USER` | 예 | `docmesh` | PostgreSQL user |
| `POSTGRES_PASSWORD` | 예 | `<password>` | PostgreSQL password |
...
현재 배포 template은 PostgreSQL 연결을 개별 `POSTGRES_*` 필드로 전달한다. dependency에는 deprecated `POSTGRES_DSN` 호환 경로가 남아 있지만 DSN과 개별 필드를 함께 전달하면 validation이 실패한다. 이 서비스의 권장 구성에서는 `POSTGRES_DSN`을 설정하지 않는다.
...
- TLS·인증서·연결 timeout 같은 세부 연결 정책은 PostgreSQL 및 배포 플랫폼 정책에 맞춰 별도 배포 구성으로 설정한다.
- password와 전체 연결 정보는 로그, API 오류 response, issue, shell history에 남기지 않는다.

### 3.2 MinIO

`MINIO_ENDPOINT`는 DNS 이름과 port로 설정한다. TLS 여부는 `MINIO_SECURE`로 지정한다.

- `MINIO_BUCKET` 문자열은 SDK 조립 시 필요하다. 현재 health check는 MinIO `list_buckets` 수준이며 지정 bucket의 존재 또는 bucket별 읽기·쓰기·삭제 권한까지 검증하지 않는다. 해당 조건은 배포 전 별도로 확인해야 한다.
...
| 변수 | 기본값 | 권장값 | 설명 |
| --- | --- | --- | --- |
| `DOCMESH_ENV` | `development` | 배포 환경 식별자 | 실행 환경 식별자 |
| `DOCMESH_HEALTHCHECK_ENABLED` | DMS factory `true`; `AppConfig` startup check `false` | 운영 배포에서는 명시적으로 결정 | DMS SDK 조립 시 health policy와 `fastapi-core` managed startup check가 같은 env 이름을 서로 다른 기본값으로 해석하므로 명시 설정 권장 |
| `MINIO_SECURE` | `true` | 운영 `true`, 신뢰된 로컬 통합 환경은 `false` 가능 | MinIO TLS 사용 여부 |

변수가 없으면 DMS environment factory의 `healthcheck_enabled()`는 `true`지만 `fastapi-core AppConfig.startup_healthcheck` 기본값은 `false`다. 저장소 `.env.example`은 placeholder endpoint로 SDK 조립 시 health check가 실패하는 것을 막기 위해 명시적으로 `false`를 사용한다. 실제 배포에서는 두 경로가 같은 정책을 사용하도록 값을 명시한다.

애플리케이션 lifespan은 생성된 DMS SDK의 `check_health()`를 필수 `dms` readiness check로 등록한다. 이 check는 `fastapi-core`가 `DOCMESH_SERVICES`에서 자동 구성하는 service-client check와 별개이며, `READINESS_REQUIRED_SERVICES`에서 `dms`를 생략해도 필수 상태로 유지된다.
...
| `CORS_ORIGINS` | `["*"]` | 명시 origin CSV | 허용 origin 목록 |
...
| `READINESS_TIMEOUT_SECONDS` | dependency 기본값 | 배포 timeout 정책 | 개별 readiness check timeout |
| `READINESS_OVERALL_TIMEOUT_SECONDS` | dependency 기본값 | 배포 timeout 정책 | 전체 readiness timeout |
...
| `DOCMESH_SERVICES` | `keycloak` | 활성 외부 service client 목록 | `fastapi-core` service client 구성 대상. 빈 문자열은 빈 목록 |
| `READINESS_REQUIRED_SERVICES` | `keycloak` | 실패 시 503을 유발할 service 목록 | 공통 service-client readiness 정책. 빈 문자열은 빈 목록 |
| `DOCMESH_SERVICE_ALTERNATIVES` | 없음 | 필요한 경우만 지정 | service alternative 구성 |

#### `ROOT_PATH`와 `TOKEN_URL`

reverse proxy route가 `/dms` prefix를 사용하면 다음처럼 설정한다.
...
| `KEYCLOAK_CLIENT_SECRET` | confidential client일 때 | `KEYCLOAK_CLIENT_PUBLIC=false`인 OAuth2 client secret |

confidential client의 `KEYCLOAK_CLIENT_SECRET`은 secret store 또는 권한이 제한된 environment로 주입한다. `KEYCLOAK_CLIENT_PUBLIC=true`이면 client secret 없이 구성할 수 있다. hard delete role `document:delete:hard`는 Keycloak role mapping에서 부여한다.
...
- 현재 애플리케이션이 직접 등록하는 항목은 SDK 집계 `dms`다. 별도 service runtime client를 선택한 경우에만 추가 detail이 나타난다.
...
다음은 reverse proxy가 `/dms` prefix를 사용하는 경우의 예다. 현재 저장소에는 이 구성을 제공하는 root Docker Compose 파일이 없으므로 배포 플랫폼에서 PostgreSQL·MinIO·Keycloak과 proxy route를 별도로 제공해야 한다.
...
1. PostgreSQL backend 사용 시 `POSTGRES_HOST`, `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD`가 있고 대상 server에 연결 가능한지 확인한다. `POSTGRES_PORT` 미설정 시 5432를 사용한다.
...
- readiness가 200이면 DMS SDK의 집계 health와 다른 필수 check가 정상이다. MinIO의 특정 bucket 권한까지 보장하지는 않는다.
- readiness가 503이면 document upload/download/delete를 수행하지 말고 `details.dms`와 correlation ID를 기반으로 장애를 조사한다.

## 7. 설정 오류와 대응

| 증상 | 가능한 원인 | 대응 |
| --- | --- | --- |
| startup 실패 | 선택 backend의 필수 연결 필드 또는 MinIO 필수 변수 누락 | 누락·공백 변수를 수정하고 재배포 |
| readiness 503 | DMS SDK health 또는 다른 필수 service check 실패 | `details.dms`, network, credential, dependency health를 점검. bucket별 권한은 별도 확인 |
...
2. `POSTGRES_PASSWORD`, `MINIO_SECRET_KEY`, `KEYCLOAK_CLIENT_SECRET`은 로그, API response, ticket, shell history에 노출하지 않는다.
