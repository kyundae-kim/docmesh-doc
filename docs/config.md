...
| `POSTGRES_PORT` | 예 | `5432` | PostgreSQL port |
...
`dms-core` v0.4.0 환경 조립은 PostgreSQL 연결을 개별 `POSTGRES_*` 필드로 받으며 `POSTGRES_DSN`을 입력으로 사용하지 않는다.
...
- readiness 응답에는 환경에 따라 개별 `postgres`/`minio` service-client detail과 SDK 집계 `dms` detail이 함께 나타날 수 있다.
...
1. `POSTGRES_HOST`, `POSTGRES_PORT`, `POSTGRES_DB`, `POSTGRES_USER`, `POSTGRES_PASSWORD`가 모두 있고 대상 PostgreSQL server에 연결 가능한지 확인한다.
...
| 증상 | 가능한 원인 | 대응 |
| --- | --- | --- |
| startup 실패 | PostgreSQL 개별 연결 필드 또는 MinIO 필수 변수 누락 | 누락·공백 변수를 수정하고 재배포 |
