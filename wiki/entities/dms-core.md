...
The package root grows from 54 to 55 names through the additive `AsyncDocumentManagementSDKFactory`. `DocumentManagementSDKFactory` is now sync-only and has no `create_async()`; native async assembly uses an `AsyncEngine` with the new factory, whose `create()` is lazy and whose `create_async()` awaits `ready()`. Wrapping a sync SDK in `AsyncDocumentManagementSDK(sync_sdk)` remains a compatibility boundary, while `from_async_components()` is the advanced direct-component path. ^[raw/articles/dms-core-wiki-api-reference-v0.9.0.md] ^[raw/articles/dms-core-wiki-api-reference-v0.10.0.md] ^[raw/articles/dms-core-wiki-examples-v0.10.0.md]

v0.10.0 adds explicit user isolation to `AccessContext`, `DmsOperationContext`, byte-upload input, and public/internal metadata. A user-scoped facade applies the same user scope to document/object namespaces, idempotency operations, cursor conditions, reads, deletion, recovery, and reset. Conflicting request/context users fail with `ValidationError`; cross-user operations fail with `AccessDeniedError`. Public metadata can expose `user_id` but still structurally excludes `storage_key`. ^[raw/articles/dms-core-wiki-api-reference-v0.10.0.md] ^[raw/articles/dms-core-wiki-examples-v0.10.0.md]
