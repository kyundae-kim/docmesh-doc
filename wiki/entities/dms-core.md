...
v0.6.0 Wiki Examples는 environment/service-config/component assembly, public/internal metadata, idempotent sync streams, structured metadata, recovery plan, health lifecycle, async stream 및 `recommended_http_error(...)` 흐름을 함께 보인다. 설치된 `dms 0.6.0`은 이 공개 표면을 제공하며 현재 adapter는 동기 upload/download를 유지하되 cursor-default 목록과 payload-size/error projection의 안전한 부분을 채택했다. async route 전환은 별도 자원 정리 contract가 필요하므로 아직 application 동작으로 확대하지 않는다. ^[raw/articles/dms-core-wiki-examples-v0.6.0.md]
