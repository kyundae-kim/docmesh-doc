...
DMS SDK를 HTTP 서비스에 붙일 때는 [[dms-core]]의 host-owned 생성 경계와 per-operation stream 정리 규칙을 custom lifespan/state 및 response 경계에 배치한다. DMS v0.9.0 facade에는 global health나 close가 없으므로 readiness와 injected client/component shutdown은 FastAPI resource/lifespan이 소유한다. 그 lifecycle과 정합성 규칙은 [[dms-core-document-lifecycle]]에 정리한다. ^[raw/articles/dms-core-wiki-api-reference-v0.9.0.md] ^[raw/articles/dms-core-wiki-examples-v0.9.0.md]

DMS v0.7.0 API reference의 `recommended_http_error(...)` 설명은 해당 release의 historical claim으로 보존한다. v0.9.0 API reference는 HTTP error response/descriptor/helper를 package-root 공개 범위에서 제외하므로, current FastAPI code must map stable DMS fields (`code`, `category`, `retryable`, optional `document_id`) to its own product envelope and status policy. ^[raw/articles/dms-core-wiki-api-reference-v0.7.0.md] ^[raw/articles/dms-core-wiki-api-reference-v0.9.0.md]
...
v0.7.0 DMS Configuration과 당시 workspace snapshot은 host가 environment/secret을 읽고 Engine·MinIO client 또는 storage component를 만든 뒤 `dms.create_sdk_from_clients`와 FastAPI `ManagedResource`에 연결하는 historical assembly를 기록한다. 이 v0.7 evidence는 현재 v0.9 package-root factory 이름이나 global close 계약을 뜻하지 않는다. FastAPI app은 계속 host-owned assembly를 resource/lifespan에 연결하지만, DMS public API 자체에 environment factory·standalone HTTP server·broker integration이 있다고 가정하지 않는다. ^[raw/articles/dms-core-wiki-configuration-v0.7.0.md] ^[raw/articles/dms-core-wiki-api-reference-v0.7.0.md]

v0.7.0 API/Examples의 public metadata, cursor continuation, explicit soft/hard delete guidance remains historical. v0.9.0 makes cursor-page listing the current public contract, adds access-aware operations and stable public projection/schema methods, and keeps `storage_key` inside internal/recovery boundaries. The HTTP adapter must preserve those boundaries without inferring DMS health, lifecycle, or HTTP status ownership. ^[raw/articles/dms-core-wiki-api-reference-v0.7.0.md] ^[raw/articles/dms-core-wiki-api-reference-v0.9.0.md] ^[raw/articles/dms-core-wiki-examples-v0.9.0.md]
