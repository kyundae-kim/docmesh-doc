...
v0.5.0 Wiki API reference도 같은 state 모델과 `create_app(config, runtime, lifespan, include_auth_router, resources, error_renderer)` 조립 경계를 기록하지만 auth router opt-in의 기본값을 `False`로 바꾼다. 현재 설치된 v0.4.0은 같은 public signature/state를 제공하되 기본값은 `True`다. `docmesh_doc.create_application`은 자신의 기본값 `True`를 `create_app`에 명시 전달하므로 현재 동작은 유지하지만, v0.5.0 upgrade 전에는 auth route 포함 여부를 테스트로 확인해야 한다. ^[raw/articles/fastapi-core-wiki-api-reference-v0.5.0.md]
...
v0.5.0 configuration reference는 동일한 assembly boundary에 startup failure mode와 retry configuration을 추가해 문서화한다. 그러나 설치된 v0.4.0 `AppConfig`에는 그 세 필드가 없고 `docmesh-py-core`도 문서의 0.4.0이 아니라 0.3.0이다. 현재 adapter는 기존 `AppConfig`와 managed resource readiness로 동작하므로, 새 startup policy는 v0.5.0 upgrade 시 version-aligned DocMesh settings와 함께 검증할 보류 항목이다. ^[raw/articles/fastapi-core-wiki-configuration-v0.5.0.md]
...
v0.5.0 Wiki Examples는 `ResourceKey` 기반 typed resource와 `get_resource(name)`을 같은 registry 경계에서 사용하는 예를 제공하며, framework-owned lifecycle이 startup·healthcheck·역순 close를 담당한다고 설명한다. 현재 adapter의 `ManagedResource(name="dms", ...)`, error mapper, explicit auth-router forwarding은 설치된 v0.4.0 signatures로 확인된다. 다만 v0.5.0 testing helper에 의존하는 runtime injection/contract test 예제는 v0.4.0에 없어 upgrade 전 보류한다. ^[raw/articles/fastapi-core-wiki-examples-v0.5.0.md]
